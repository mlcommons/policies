...
== Numerical precision must not be used to disparage or diminish valid MLPerf results

Each MLPerf benchmark defines specific accuracy or quality targets that every valid submission must meet. All valid MLPerf submissions — regardless of the lowest numerical precision used — satisfy the same accuracy or quality requirements for that benchmark. Therefore, the use of lower numerical precision in a valid submission does not indicate inferior result quality.

Submitters may optionally disclose the lowest numerical precision used in their submission. Whether or not such disclosure is made, the following rules apply:

  - Submitters may not imply or state that a result using a particular numerical precision (e.g., FP8, FP4) does not sufficiently converge, produces lower-quality outcomes, or is otherwise less valid than a result using higher precision, provided both results are valid MLPerf submissions meeting the required accuracy or quality target.
  - When comparing results that differ in lowest numerical precision, submitters are encouraged to identify the precision difference (consistent with the requirement in the preceding section to identify submission differences).

**Compliant Example**:

____
SmartAI Corp achieved a score of 0.6 on the MLPerf® Training v4.0 Image Classification benchmark using FP8 precision on a SmartCluster with 8 chips [1], which is faster than the score of 1.2 achieved by LessSmartAI Corp using FP16 precision with 16 chips [2].
____

**Non-Compliant Example**:

____
SmartAI Corp achieved a score of 0.6 on the MLPerf® Training v4.0 Image Classification benchmark using a SmartCluster, but this result used FP8 precision which does not converge as well as results using higher precision and thus cannot be compared to a score of 1.2 achieved by LessSmartAI Corp using FP16 precision.
____

The non-compliant example above violates these Guidelines because it implies that a valid MLPerf result achieved using FP8 precision is inferior due to the precision used. But both results met the same required accuracy target and are equally valid MLPerf submissions.
