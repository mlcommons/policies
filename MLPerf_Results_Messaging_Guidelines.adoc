...
\* disparage or diminish means to negatively position another submitter's results 

**Non-Compliant Example**:

____
LesserSmartAI Corp will only compare against FP32 results submitted by SmartAI Corp because FP32 precision is best suited for high-accuracy training. BF16 is not yet usable for large scale production training so LesserSmartAI Corp will not compare to SmartAICorp's BF16 results.
____
