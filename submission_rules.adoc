...
*   One representative of each submitter
...
The review committee is chaired by the relevant results working group chair
...
issue. Non-submitters may not vote, except in the case of a tie. In the case of
a tie, the first non-submitter from the following list will cast the deciding
vote: the review chair, the chairs of the relevant working group shortest tenure
first, the chairs of the relevant power working group shortest tenure first, the
president of the board, the executive director, and a random number generator.
...
Because the submission round is confidential to the submitters, the review
committee agenda, deliberations, and specific decisions are confidential and
shared only with committee members and submitters for that round. The general
nature of decisions may be shared outside the review process because such
decisions may expose the need for rules changes.

The private submission repo will be deleted when the next MLPerf submission is
made public or after 200 days.
