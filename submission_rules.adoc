...
# General MLPerf Submission Rules v0.3
...
### Agenda and decisions [[id_agenda_and_decisions]]
...
than 2% cumulative difference, or improve its position in the competitive standings 
that are not covered by <<id_qualified_exceptions,Qualified Exceptions>> require 
unanimous consent. 

Otherwise, if there are two outcomes, voting proceeds by simple majority. If there are more
...
The MLPerf review committee reserves the right to amend its schedule and/or
...
### Submission content [[id_submission_content]]
...
The submission must be submitted by the submission deadline either as a pull request to the submission repository or using the <<id_submission_privacy,private submission process>>. 

#### Qualified Exceptions [[id_qualified_exceptions]]

In the case where last-minute unforeseen events prevent a submission from being submitted to the submission repository or sent to the results chair, a Late Submitter is allowed to make a submission after the deadline following the procedure outlined below. This procedure is meant for extreme circumstances beyond the submitter's control. No submitter may make a Late Submission in two consecutive rounds unless by unanimous approval from the other submitters.



*   Prior to the submission deadline, the Late Submitter must provide a hash of a tarball containing the submission as Proof of Work as well as an explanation of the reason for the delay via email to the other submitters as well as the results chair, so that the other parties can anticipate a late submission. The submission tarball must contain <<id_submission_content, all required content>>. 
*   Once notified of a late submission, the chair will wait until all late submissions are resolved before releasing any of the <<id_submission_privacy,private submission results>>.
*   The submitter then has 24 hours after the submission deadline to make the submission tarball available to the results chair and other submitters. The tarball must match the hash. If the hash has been verified and there are no objections from other submitters regarding the provided explanation, the results chair may accept the submission and make it and all other submissions available.  Otherwise the review committee must decide whether the Late Submission meets the criteria for acceptance, following the procedure described <<id_agenda_and_decisions,here>>.
*   Failure to comply with the late submission procedure outlined here will result in the submission being deemed invalid except by unanimous approval from other submitters based on compelling explanation of extenuating circumstances. Examples of such cases include:
**  Failure to provide the hash of the submission tarball before the submission deadline.
**  Failure to provide a reason for the delay before the submission deadline.
**  Failure to provide the submission tarball within 24 hours of the submission deadline.
...
| Depends on private source code from an internal source control system | Unique source identifier [i.e., gitlab hash, p4 CL, etc] | No | Yes. Should be made "Available" within 180 days of the submission date, or by the next MLPerf submission date, whichever is longer | Yes
| Depends on partially redacted source code from an internal source control system (line numbers logged in result files should comply with redacted source code for easy review)  | Unique source identifier [i.e., gitlab hash, p4 CL, etc] | No | Yes. Should be made "Available" within 180 days of the submission date, or by the next MLPerf submission date, whichever is longer | Yes
| Private binary | Checksum | No | Yes. Should be made "Available" within 180 days of the submission date, or by the next MLPerf submission date, whichever is longer | Yes
|===
...
### Submission Privacy [[id_submission_privacy]]
