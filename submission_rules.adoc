...
A research, development, or internal (RDI) component  does not meet the requirements for an available or preview component. An RDI system is a system containing one or more RDI components. RDI hardware components may not be submitted as _Available_ components  until the submission cycle after next or 221 days whichever is longer.

RDI systems using only Available hardware must use a _Reproducible_ software stack.  A _Reproducible_ software stack consists only of _Available_ components and _Reproducible_ binary components. A binary component is _Reproducible_ if it is downloadable and installable by the submission deadline from a URL by anyone to whom the hardware is Available, which must remain valid until a release of the software is Available.  The binary must be accompanied with instructions to allow a reasonable expert user to install it on a hardware system similar to the submission system.
