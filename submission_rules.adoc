...
A _Preview_ system may contain a newly developed hardware component (e.g. a new ML accelerator) that is a substantial contributor to the determination of ML performance. "Newly developed" is defined as not being available, by the MLPerf definition of _Available_, prior to the submission date of the previous MLPerf round. For example, a chip that was available prior to the submission date of the previous MLPerf round but was never used before for an MLPerf submission does not qualify as "newly developed."  A newly developed hardware component that is a substantial contributor must be either an accelerator chip, CPU, or NIC. In a submission using a _Preview_ system with newly developed hardware, the “Available software stack” requirement is waived for software that is necessary to support that component. Otherwise, _Preview_ systems must meet the same _Available_ software stack requirements as an _Available_ system. Note that "newly developed" applies to all SKUs of a chip. The first shipping versions of a new accelerator introduced within one round of MLPerf need not meet the _Available _software stack requirements, but subsequent SKUs of that accelerator released in subsequent rounds of MLPerf are not considered newly developed, and must meet _Available_ software stack requirements.
