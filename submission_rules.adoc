...
During the review period, all submission details and results are strictly confidential and shared only with the review committee after the submission deadline. Access to these results is granted as a privilege of submitting, and submitters are expected to maintain confidentiality until publication. Given that individuals serving a role demarcated by † could be from a non-submitting organization, other submitters may request that such an individual should not have early access to submissions. In this case, the relavant working group along with the head of MLPerf must make best effort to address the request.
