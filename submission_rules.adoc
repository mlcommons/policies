...
For large scale training submissions (using 1K or more accelerators/processors), these submissions are excluded from the standard stack rank based review assignment by the number of results. Instead, submitters with the highest scale submissions will review each other, with assignments determined by stack rank on the scale of submission. 
