...
For 0.7, software can be considered Available if it is generally available by the publication date for benchmark results, provided the same software stack is used for the submission, this software stack is made available to all MLPerf auditors during the review period, and there are no material or substantial changes to the software stack between submission and publication.  This is intended to allow for final QA to happen between release and submission to mitigate the variation in length of QA cycles for different vendors.
