...
using this [tool](https://github.com/mlperf/inference/blob/master/tools/submission/truncate_accuracy_log.py).
The tool will take a hash of the original mlperf_log_accuracy.log, add the hash to accuracy.txt and than truncate mlperf_log_accuracy.log.
A submiter would run the tool before submitting to mlperf and ***keep*** the original mlperf_log_accuracy.log files inside their organization.
The original files might be requested by mlperf during submission review so you need to store them.
Run the tool the following way assuming SOURCE is your local subumission tree and DEST the location of the github submission repo:
