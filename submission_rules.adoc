...
The submission schedule will be set yearly, and must be approved by both the inference and training submitters meetings.  In general, the goal is to have two Inference and two Training submissions each year, and to not have them overlap eachother.  MLCommons attempts to avoid major international holidays, and accomodate relevant conferences.
...
. *Domain Identification* - Working groups review proposals for domain adds/removals from members.  The working group should attempt to come to majority concensus in 1 or 2 meetings.  If concensus cannot be had, this should go to a vote according to the MLCommons voting rules.
. *Sync Domains* across working groups (ie Inference, Training, and HPC)
. *Identify PIC* (person in charge) to drive this domain addition across all working groups
. For each domain addition, do the two following steps in parallel:
.. *Advisory Board Formation* for the domain.  
.. *Task Force(s) create benchmark proposals.*  The task force(s) should consider all working groups that might consume this benchmark (ie Inference and Training).  Ideally benchmark proposals should take around 2 months or less.
. *Review benchmark proposals* with the Advisory Board.  The board approves, rejects, or suggests changes.
. *Formal working group acceptance* - the working group needs to come to concensus on whether or not to accept the benchmark proposal that has now been approved by the Advisory Board.  If concensus cannot be had, this should go to a vote according to the MLCommons voting rules.

Working groups are encouraged to start this process early, possibly several rounds ahead of time to account for larger changes.  For example Domain Identification could be done for several rounds at once.  Some working groups such as HPC may choose to replace the Advisory Board formation with working group concensus, but in general working groups should try their hardest to get third party opinions from non-submitters.
